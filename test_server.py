        ...
        if fragment_message.is_all_0() or fragment_message.is_all_1():
            ...
            if fragment_message.is_all_1():
                ...
                # If they are the same, start the reassembling proccess
                    ...
                    for i in range(2 ** m):
                        for j in range(2 ** n - 1):
                            fragment_file = read_blob(BUCKET_NAME, "all_windows/window_%d/fragment_%d_%d" % (i, i, j))
                            print("fragment_file -> {}".format(fragment_file))
                            # fragment = Fragment(profile_uplink, fragment_file)
